...
\todo[inline]{Need moderator block distance from S1 if we have it.}
...
\todo[inline]{Insert measurement of the required shift for utof here.}
...
A further correction is applied to this measurement to give Figure~\ref{fig:s4tof}.
...
\begin{figure}[h]
  \begin{minipage}[t]{0.49\textwidth}
    \begin{adjustbox}{max totalsize={\textwidth}, center}
      \input{files/Figures/barEffEx.tex}
    \end{adjustbox}
    \caption{Relative efficiency vs. horizontal position along an $\mathit{S4}$ bar. This data is for the 0 moderator block case.}
    \label{fig:s4PosEff}
  \end{minipage}
  \hfill
  \begin{minipage}[t]{0.49\textwidth}
    \begin{adjustbox}{max totalsize={\textwidth},center}
      \input{files/Figures/barEffs.tex}
    \end{adjustbox}
...
Separate to this, the measurement is also corrected for the efficiency as a function of the distance along the $\mathit{S4}$ bar.
This relative is efficiency is derived as follows.
It is assumed that rate of cosmics passing through each part of a given $\mathit{S4}$ bar is uniform.
The number of cosmics through each $\mathit{S4}$ bar as a function of position is measured by assuming that all of the signals occurring outside of beam spills are produced by cosmic rays.
The efficiency is then derived from this distribution by scaling it such that the histogram bin with the highest number of cosmics has a value of 1.
An example of one of these distributions is shown in Figure~\ref{fig:s4PosEff}.
MIP and proton signals hits are then scaled by the reciprocal of this efficiency, depending where along a given bar this hit occurs.
Figure~\ref{fig:s4PosEff} shows that the relative efficiency of the bar drops from 1 at the bar centre (70~cm) to less than 0.05 at the ends of the bar.
