...
\todo{JOCELYN:"really only these two days?  This paper needs to be the flux reference, so we also need to include the flux measurements during the beam test....
...
\todo{SEB, I changed this paragraph in accordance with JOCELYN's comments, but please have a read through. We also need the off axis angle (with +/-). MORGAN made an updated version of this schematic with the axis labels. Need to find it and add it. Need to check the mod block from S1}
...
The \textit{S2} counter was placed 1.4m +/- xm \todo{THIS NUMBER NEEDS CHECKING - SEB?} downstream of \textit{S1}.
...
\todo{JOCELYN:"with what threshold? specify" SEB? JOCELYN:"was it really only one of the S1 PMTs?
I thought a coincidence of the S1 PMTs was formed, and then that signal was ANDed with the S2 PMT." SEB?}
...
\todo{JOCELYN:"Give exact detector area" - SEB?}
\begin{figure}
  \centering
  ...
  \label{fig:S3sketch}
\end{figure}
\todo{JOCELYN:"its not clear from this description that the bars are staggered in z, include a side view in Fig. 5 to clarify."-SEB?}
...
\todo{JOCELYN:"give the scintillator brightness first (usually in photons per MeV deposited), then the attenuation length " - SEB?}
The scintillation emission spectrum of EJ-200 resides primarily in the violet region of the visible spectrum that corresponds well to the photon detection efficiency wavelength range of photosensors used for the readout. \todo{JOCEYLN: "Add a sentence describing how the bars were wrapped (with what, why, with what reflectivity, etc.)" - SEB?}
\todo{JOCELYN:"give the wavelength range numerically here, and include a citation to the data sheet that shows the spectrum" -SEB?}
...
\todo{JOCELYN:"What were the conditions of the self trigger i.e. threshold? multiplicity? time over threshold? etc.) specify here." SEB?}
...
The discriminated signals are then fed into a time-to-digital converter. A signal in \textit{S4} was observed to have occurred if a signal was seen in both PMTs, above the discriminator threshold, on the same bar within 20~ns of each other. 

The $\mathit{S1-S2}$ coincidence signal was digitized by the same time-to-digital converter. This signal was used to calculate the particle time of flight from $\mathit{S2}$ to $\mathit{S4}$.
...
The primary aims of the use of $\mathit{S1} - \mathit{S4}$ were to make measurements of the flux of particles entering and exiting the TPC vessel, and to perform particle identification using the time of flight measurement.
...
It is also possible to provide a calculation of the multiplicity of protons reaching the geometric area of the TPC active region.
...
\begin{figure}[h]
  \begin{adjustbox}{max totalsize={.7\textwidth}, center}
    \input{files/Figures/TvsA_2block.tex}
  \end{adjustbox}
