...
The data is corrected for the efficiency of each of the \textit{S4} bars and a background is subtracted from the data. % New paragraph here with discussion of how this is done
Initially, timing delays caused by cabling and equipment are taken into account. % More detail needs to be added here
Then a Gaussian is fitted to the peak caused by faster particles.
All times are normalised such that the mean of this peak is at a value of $\mathit{S4} - \mathit{S2}$ corresponding to a particle traversing the distance between $\mathit{S2}$ and $\mathit{S4}$ at the speed of light. 
