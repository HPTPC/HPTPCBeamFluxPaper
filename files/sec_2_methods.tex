...
\begin{figure}
  \includegraphics[width=1.0\linewidth]{files/Figures/hptpc_t10_planview.jpg}
...
\begin{figure}
  ...
  \caption{Photo illustrating the $\mathit{S1}$ and $\mathit{S2}$ counters and the stand with four acrylic moderator blocks (right). Photo of the downstream part of the setup which shows the $\mathit{S3}$, $\mathit{S4}$ detectors and HPTPC (left).}
  \label{fig:modblocks}
\end{figure}
...
The TPC was placed 13~m downstream of the BPM and 0.67~m away from the beam centre, so that the TPC active region spanned off-axis angles 1.4--3.8$^{ \circ }$.
...
Directly between $\mathit{S1}$ and $\mathit{S2}$, a variable number of blocks of acrylic were placed, and are shown in figure~\ref{fig:modblocks}.
Both the TPC and ToF systems were placed at an off-axis angle with respect to the direction of the beam (see section~\ref{sec:coord} for further details).
To study the impact of moderator thickness, a variable number of blocks of acrylic, each of dimension $10\times10\times10$~cm$^3$, were placed in the beamline and are shown in figure~\ref{fig:modblocks}.

The data acquisition (DAQ) systems of the upstream ($\mathit{S3}$) and downstream ($\mathit{S4}$) ToF systems were completely independent.
...
T10 received between 1 and 3 spills from the PS during each supercycle, which has a typical duration of 33~s.
...
Up to four acrylic $10\times10\times10$~cm$^3$ blocks could be placed contiguously on a tripod stand.
...
\label{sec:coord}
The T10 beamline area was surveyed, and the distances to various components measured to a precision of 0.5~mm, by the CERN Survey, Mechatronics and Measurements (SMM) group.
Multiple points on each of $\mathit{S1}$, $\mathit{S2}$, $\mathit{S3}$, $\mathit{S4}$ and the TPC frame have had their positions measured relative to an origin located at the nominal T10 beam focussing point.
...
\begin{table}
  ...
  \label{tab:angS1}
\end{table}
\todo{Morgan: is the uncertainty on the TPC angles justified? Those points were not measured by the survey team directly. This applies to table 2 as well.}
...
\begin{table}
  \centering
  \caption{Distances between centres of objects in the T10 beamline. US and DS refer to upstream and downstream respectively.}
  \begin{tabular}{|c|c|}
    \hline
...
The detector is composed of 22 staggered scintillator bars:  20 bars with dimensions $168 \times 6.0 \times 1.0$~cm$^3$ and 2 bars of  $1 \times 6.0 \times 1.0$~cm$^3$ placed on top and bottom~\cite{S3-proceedings}.
...
\begin{figure}[t]
  \begin{minipage}[t]{0.49\textwidth}
    ...
    \end{adjustbox}
    \caption{Reconstructed spatial distribution of hits observed in $\mathit{S3}$. In this case a timing cut has been applied to select only those hits identified as coming from minimum ionizing particles. These particular data were taken without a moderator in the beamline.}
    \label{fig:s3XY_pion}
  \end{minipage} 	
  \hfill
  \begin{minipage}[t]{0.49\textwidth}
    ...
    \end{adjustbox}
    \caption{Reconstructed spatial distribution of hits observed in $\mathit{S3}$. In this case a timing cut and an amplitude cut have been applied to select only those hits thought to come from protons. These data were taken with 4 moderator blocks in the beamline.}
...
the steel vessel is rated to 6~barA of pressure, and the walls of the vessel are 1~cm thick. The angular position of the TPC can be found in table~\ref{tab:angS1} while its position relative to other objects in the beamline can be found in table~\ref{tab:distances}.
 
The TPC comprised thin steel mesh electrodes (one cathode and three anodes), and 12 copper rings to create the uniform drift field. The diameter of the field cage was 111~cm, with a shortest distance to the vessel of 14.5~cm.
...
\begin{figure}
  ...
  \caption{Cross-sectional view of the TPC; the thin mesh electrodes and copper ring drift volume can be seen inside the steel vessel. The walls of the vessel shown are 1~cm thick with a vessel outer diameter of 142~cm. At the point of hitting the vessel, the beam centre was 1.14cm below the centre of the vessel vertically, where the distance from the inside of the vessel wall to the drift region was 15~cm.}
   \label{fig:TPC}
...
More data is taken for 4 blocks as that was the configuration used for the majority of the beam test.
...
\begin{align}
  t_{f,~\text{rec}} = t_f + t_{f,~\text{delay}} \,,
  \label{eq:delayS4}
\end{align}
...
\begin{align}
  t_{i,~\text{rec}} = t_{i} + t_{i,~\text{delay}} \,,
  \label{eq:delayS2}
\end{align}
where $t_{f,~\text{delay}}$ and $t_{i,~\text{delay}}$ are the total delays caused by cabling and equipment between the respective detectors and the DAQ.
Therefore, the true time of flight, $t_{f} - t_{i}$, will be given by
\begin{align}
  t_{f} - t_{i} = t_{f,~\text{rec}} - t_{i,~\text{rec}} - \left( t_{f,~\text{delay}} - t_{i,~\text{delay}} \right) \,,
  \label{eq:delayTof}
\end{align}
where $t_{f,~\text{delay}} - t_{i,~\text{delay}}$ is to be determined.
In order to determine $t_{f,~\text{delay}} - t_{i,~\text{delay}}$, a Gaussian is fitted to the faster peak observed in the 0 block data which is thought to be produced by minimum ionizing particles.
...
Rearranging equation~\ref{eq:delayTof} and substituting in 86.31~ns for $t_{\mathit{S4},~\text{rec}} - t_{\mathit{S2},~\text{rec}}$ gives a value for $t_{\mathit{S4},~\text{delay}} - t_{\mathit{S2},~\text{delay}}$ of 43.7~ns.
...
\begin{figure}[ht]
  \begin{minipage}[t]{0.49\textwidth}
    \begin{adjustbox}{max totalsize={\textwidth}, center}
      \input{files/Figures/tvsa0.tex}
    \end{adjustbox} \\
    \begin{adjustbox}{max totalsize={\textwidth}, center}
      \input{files/Figures/tvsa2.tex}
    \end{adjustbox}
  \end{minipage} 
  \begin{minipage}[t]{0.49\textwidth}
    \begin{adjustbox}{max totalsize={\textwidth}, center}
      \input{files/Figures/tvsa1.tex}
    \end{adjustbox} \\
    \begin{adjustbox}{max totalsize={\textwidth}, center}
      \input{files/Figures/tvsa3.tex}
    \end{adjustbox}
  \end{minipage}  
...
\begin{figure}[ht]
  ...
  \end{adjustbox}
  \caption{$\mathit{S4}$ time-of-flight spectra for varying numbers of moderator blocks. For all configurations, an exponentially falling background has been fitted and subtracted from the data. Additionally, the plot has also been corrected for the differing efficiencies of the various bars and for the variation in efficiency as a function of position along the bar.}
  \label{fig:s4tof}	
\end{figure}
...
The distribution is again produced using equation~\ref{eq:recoMass}.
...
The number of cosmic rays passing through each 7~cm segment of each $\mathit{S4}$ bar is measured by assuming that all signals occurring outside of beam spills are produced by cosmic rays.
The efficiency is then derived from this distribution by scaling with a constant factor, that the bin with the highest number of cosmics has a value of 1.
