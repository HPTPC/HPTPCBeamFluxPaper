...
\begin{figure}[ht]
  \begin{minipage}[t]{0.49\textwidth}
    \begin{adjustbox}{max totalsize={\textwidth}{.5\textheight},center}
      \input{files/Figures/angularDist.tex}
    \end{adjustbox}
...
The $\mathit{S2}$ counter was placed $(1.41 \pm 0.01)~\text{m}$ \todo[inline]{THIS NUMBER NEEDS CHECKING - SEB?} downstream of $\mathit{S1}$.
...
\label{subsec:s4Exp}
The $\mathit{S4}$ `downstream' ToF constituent sat immediately downstream of the HPTPC prototype in the beamline. \todo[inline]{JOCELYN: ``replace with a number and an error bar''}
...
\todo[inline]{JOCELYN: How are the timing ranges chosen? What are the timing ranges for protons and MIPs? Suggest include a table with the ranges for teh uToF, dToF and the analytic expectation in bith given the distances.}
\begin{figure}[h]
...
\begin{figure}[h]
  \begin{adjustbox}{max totalsize={.7\textwidth}, center}
    \input{files/Figures/TvsA_4block.tex}
  \end{adjustbox}
  \caption{Example of SiPM signal amplitude plotted against uncorrected $\mathit{S3}$ time of flight. The population in the upper right is produced by protons. This particular figure shows the distribution when 4 moderator blocks are present.}
  \label{fig:TvsA}
\end{figure}
