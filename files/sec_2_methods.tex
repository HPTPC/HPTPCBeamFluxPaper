...
The minimum separation in time between two spills is 1 second, so the start-of-spill signal frequency is 1 Hz or smaller.
...
\begin{figure}[ht]
  ...
  \end{adjustbox}
  \caption{Angular position of various objects within the T10 beamline; the beam is coming out the page in this view. US and DS refer to the upstream and downstream faces of the HPTPC. The origin in this view is at the centre of $\mathit{S1}$; the true centre of the steered beam is at +1$^{\circ}$ in $\theta$ and 0$^{\circ}$ in $\phi$.}
...
The scintillation emission spectrum of EJ-200 resides peaks in the violet region of the visible spectrum (435~nm)~\cite{EJ200}, which corresponds well to the photon detection efficiency wavelength range of the photosensors used for the readout.
...
A second level trigger was implemented in firmware and run on the level of the ASICs: the data were only sent to the hard disk of the DAQ computer if there was a coincidence between the $\mathit{S1}$ channels and at least three channels in the ASICs used for $\mathit{S3}$.
...
Examples of reconstructed $XY$ distributions are shown in figure~\ref{fig:s3XY_pion}.
...
The angular position of the center of the TPC is approximately 2.5$^{\circ}$. 
More details of the position and extent of the TPC are given in table~\ref{tab:angS1} and table~\ref{tab:distances}.
 
The active TPC is a cylinder 11~cm in diameter and 48~cm in length; the TPC comprised thin steel mesh electrodes (one cathode and three anodes), and 12 copper rings to create the uniform drift field. 
...
A particle of mass $m$ with momentum $p$ will traverse a distance $d$ a time
\begin{align}
...
The proton peaks show a double peak feature, with a smaller delayed peak closely following the main proton peak; this feature appeared after the beam was steered so that the full 2.5 degree off-axis able could be achieved and is due to beam scattering in the steering magnets.
Figures 3 and 4 show the proton peak for unsteered beam and the double peak structure is gone.
In the black curve, which shows 0 block data, the peak at 95~ns corresponds to deuterons.
...
The efficiency is then derived from this distribution by scaling with a constant factor found by letting the bin with the highest number of cosmics have a value of 1.
