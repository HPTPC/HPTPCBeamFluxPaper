...
However, the addition of the fourth moderator block led the number of protons passing through the TPC to fall back down tos ($21.4 \pm 0.2$) per spill.
...
Figure~\ref{fig:thetas4mip} also shows that initially, an increasing number of moderator blocks results in an increased total MIP flux through $\mathit{S4}$. 
However, upon the addition of the fourth moderator block the flux of MIPs is seen to fall.
This is observed due to both $\mathit{S2}$ and $\mathit{S4}$ being positioned off-axis, avoiding many of the unmoderated beam particles striking these detectors.
...
As the precise initial beam characteristics were not readily available, particle positions and momenta were drawn from the $\mathit{S3}$ distributions shown in figure~\ref{fig:s3promom}.
...
Table~\ref{tab:systematics} shows the systematic uncertainties on the number of protons measured in $\mathit{S3}$ and $\mathit{S4}$ in both data and Monte Carlo.
The systematic uncertainty on the MC simulations is determined by varying a number of geometric factors within the simulation including the position of the $\mathit{S1}$ and $\mathit{S2}$ detectors.
These variations induce changes in the direction and momenta of the propagated protons.
Additionally, simulations are run with 1~cm of additional acrylic in the beamline.
We are letting these variations stand in for a number of geometry uncertainties.

For the data systematics, the uncertainty on the $\mathit{S4}$ angular correction is determined by varying the number of horizontal bins in figure~\ref{fig:s4PosEff} from 20 to 10.
...
This has a large effect in the 4 block case because of the very small number of protons detected in $\mathit{S4}$ relative to the background.
...
The agreement shown in the 0, 1, 2, and 3 block cases relative to the uncertainty provided by the beam test setup provides strong evidence that efficiency corrections described in section~\ref{timeofflightanalysissubsec} are justified; it is not known if additional small quantities of light moderating material were present in the beamline which are not accounted for in the statistical errors.
