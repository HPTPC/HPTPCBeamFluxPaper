...
\label{hptpcPaper:sec:Results}
	...
	This flux is shown as a function of the number of moderator blocks and as a function of the angle in the $x-y$ plane (see figure~\ref{fig:setup} for the definitions of these axes).
	For all moderator blocks, the peak number of minimum ionizing particle events occurs at a value of $\theta$ between $1^{\circ}$ and $2^{\circ}$.
	...
	The limited efficiency of $S4$ at the ends of the bars (near to one of the photomultiplier tubes) means that fewer signal hits are recorded at these points. 
	...
	Figure~\ref{fig:beamAng} shows that, when observed from $S1$, a limited area of $S4$ is covered by the active area of $S2$.
	Given the requirement that a signal must be observed in $S2$, this means that the majority of events that are observed in $S4$ will be those that are in this overlap region.
   
   	Figure~\ref{fig:thetas4mip} also indicates that an increasing number of moderator blocks results in a increased total MIP flux through $S4$.
   	This is observed due to the off-axis position of both $S2$ and $S4$ which means that much of the unmoderated beam does not strike these timing points.
   	Due to scattering processes in the moderator, a greater number of MIPs are incident upon $S2$ and $S4$, with more scattering occuring in greater moderator thicknesses.
   	...
   	The initial increase is thought to be for similar reasons to those specified for the MIP flux, with increased scattering meaning that more protons pass through the off-axis $S2$ and $S4$.
   	The later fall may be due to the larger loss of energy of the protons in the thicker moderator.
	...
	This flux is then normalised such that its area is equal to 1.
	The factors are then calculated by integrating this distribution between the angular limits of the other timing points.
	...
	\begin{figure}[h]
		...
		\begin{minipage}{0.49\textwidth}
			\begin{adjustbox}{max totalsize={\textwidth}{0.5\textheight}, center}
				\input{files/Figures/absFluxXAngleS13.tex}
			\end{adjustbox}
